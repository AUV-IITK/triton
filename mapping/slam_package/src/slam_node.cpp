...
SlamFilter::SlamFilter(int n): num_particles_(n) {
	...
	unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
	gen_ = std::default_random_engine(seed);
	vision_subscriber = nh_->subscribe("vision/buoy", 10, &SlamFilter::Landmark, this);
	odom_subscriber = nh_->subscribe("/odometry/filtered", 10, &SlamFilter::Update, this);
}
//SlamFilter is for robots pose and SlamParticles are for the landmarks
